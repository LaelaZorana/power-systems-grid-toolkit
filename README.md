...
Grid analysis usually arrives as a sealed commercial package, which makes it easy to assume the algorithms inside are exotic. They aren't. The core of it is linear algebra, so this library builds the whole thing from the equations up in numpy and scipy: steady state and dynamic power system analysis with renewables integration studies on top, meaning PV and wind models, hosting capacity sweeps and battery storage dispatch. The load flow, fault, dispatch and stability modules get checked against published numbers from the IEEE test systems and textbook worked examples, and the renewables and storage models only get checked for internal consistency, because there's no published benchmark to pin them to. The 14-bus voltages match the IEEE Common Data Format printed solution to 0.13 percent in magnitude and 0.02 degrees in angle, so you can set the printed table beside the solver output and read down both columns.
